...
export default task('SafleIdFullLifecycleTest', 'Test full lifecycle for SafleId registration and updates').setAction(
    async (_, hre: HardhatRuntimeEnvironment) => {
        ...
        const createSignature = async (signer: any, dataTypes: string[], dataValues: any[]) => {
            const encoded = hre.ethers.utils.defaultAbiCoder.encode(dataTypes, dataValues)
        ...
        const regData = hre.ethers.utils.defaultAbiCoder.encode(['string'], [safleId4])
        const regSig = await signer2.signMessage(hre.ethers.utils.arrayify(hre.ethers.utils.keccak256(regData)))
