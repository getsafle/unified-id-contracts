...
task("generate-sigs", "Generate signatures for SafleID functions").setAction(
  async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    const [master, primary, secondary, newPrimaryWallet] =
    ...
    console.log(
      "Encoded Data:",
      encode(["string", "address"], [safleId, master.address]),
    );
    ...
    console.log(
      "initiateUpdateSafleId:",
      await sign(master, ["string", "address"], [newSafleId, master.address]),
    );
