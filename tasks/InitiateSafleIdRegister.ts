...
export default task(
  "InitiateSafleIdTests",
  "Test all initiate functions",
  ...
  const createSignature = async (
    ...
    const message = hre.ethers.utils.solidityKeccak256(
      ...
      params.map((p) =>
        typeof p === "string"
          ? hre.ethers.utils.hexlify(hre.ethers.utils.toUtf8Bytes(p))
          : p,
      ),
    );
  ...
  try {
    ...
    const updatePrimarySig = await createSignature(signer1, "updatePrimary", [
      safleId1,
      signer2.address,
    ]);
    try {
      const updatePrimaryTx =
        await RegistrarStorageChildContract.initiateUpdateSafleIdPrimaryAddress(
          safleId1,
          signer2.address,
          updatePrimarySig,
          "0x",
      ...
      console.log(
        "Update Primary Transaction Hash:",
        updatePrimaryReceipt.transactionHash,
      );
    } catch (error) {
    ...
    try {
      const registeredSafleIds =
        await RegistrarStorageChildContract.getRegisteredSafleIds();
